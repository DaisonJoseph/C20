...
//	$user = $db->user; if(!isset($_SESSION['Uid'])){header('Location:login.php');} 
	...
	// $collection->remove();
	// $command = 'mongoimport --db ficoh_db --collection masterinput --file C:/xampp/htdocs/new/masterinput.csv --type csv --headerline' ;
	// shell_exec($command);	

	// $count1 = $collection -> find() -> count();
	// echo 'count1 ' . $count1;

	// $collection = $db->inboundinterfaces;	
	// $collection->remove();
	// $command = 'mongoimport --db ficoh_db --collection inboundinterfaces --file C:/xampp/htdocs/new/inboundinterfaces.csv --type csv --headerline' ;
	// shell_exec($command);

	// $count1 = $collection -> find() -> count();
	// echo 'count1 ' . $count1;

	// $collection = $db->outboundinterfaces;	
	// $collection->remove();
	// $command = 'mongoimport --db ficoh_db --collection outboundinterfaces --file C:/xampp/htdocs/new/outboundinterfaces.csv --type csv --headerline' ;
	// shell_exec($command);
	...
	elseif ($appage == "6-10 Years" or $appage == "< 10 Years" ) 
		...
		// $riskcriticalityindex1 = 2*LOG(1.1*$apprevenueimpact,2);
		// $riskcriticalityindex2 = 2*LOG10(1.1*$appcriticality);
		// $riskcriticalityindex3 = 2 * (LOG(1+$appdatagatewayvalue,2));
		
		// $riskcriticalityindex4 = LOG10(1+$applegalvalue)/2;
		// $riskcriticalityindex5 = LOG10(1+$appexternalconnectivityvalue)/2;
		
		// echo '<br> risk new value1 ' . $riskcriticalityindex1;			
		// echo '<br> risk new value2 ' . $riskcriticalityindex2;			
		// echo '<br> risk new value3 ' . $riskcriticalityindex3;
		// echo '<br> value 3 ' . $appdatagatewayvalue;
		// echo '<br> risk new value4 ' . $riskcriticalityindex4;			
		// echo '<br> risk new value5 ' . $riskcriticalityindex5;			
		...
		$m->executeBulkWrite('allianz.masterinput', $bulk);
